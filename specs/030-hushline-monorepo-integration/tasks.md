...
- [ ] T001 브랜치 `030-hushline-monorepo-integration`에 설계·스펙 문서 커밋 (`docs/20260721-acp-agent-core-reuse-strategy.md`, `specs/030-hushline-monorepo-integration/*`)
- [ ] T002 병합 전 레이아웃 대조: `pnpm-workspace.yaml`, 루트 `Cargo.toml`, `turbo.json`을 hushline 레포와 비교해 편입 지점 확정 (참조: `docs/20260721-acp-agent-core-reuse-strategy.md`)
...
- [ ] T003 hushline을 `apps/hushline`로 히스토리 보존하며 import (앱 패키지명 `@yoophi/hushline` 유지)
- [ ] T004 루트 `Cargo.toml` workspace `members`에 `apps/hushline/src-tauri` 추가
- [ ] T005 `turbo.json` 파이프라인/필터에 hushline 태스크(dev/build/tauri/test) 반영 및 루트 `package.json` 스크립트 정리
- [ ] T006 `pnpm install` 후 편입 배선 초기 빌드로 워크스페이스 인식 확인

### Group B — 공유 코어 추출 (US2/US3 선행, workbench 무동작 리팩터)

- [ ] T007 `crates/acp-agent-core` 생성(`Cargo.toml` edition 2021 핀) 및 루트 `Cargo.toml` workspace member 등록
- [ ] T008 [P] `agentic-workbench`의 domain 모듈을 `crates/acp-agent-core/src/domain`로 이동 (run, events, agent, permission, acp_session, agent_tool_candidate)
- [ ] T009 [P] ports 모듈을 `crates/acp-agent-core/src/ports`로 이동 (session_launcher, session_handle, session_registry, event_sink, acp_session_store, agent_catalog, permission)
- [ ] T010 application 유스케이스를 `crates/acp-agent-core/src/application`로 이동 (start_agent_run, send_prompt, cancel_agent_run, set_permission_mode, cancel_prompt_and_send, steer_prompt, agent_run_errors)
- [ ] T011 infrastructure를 `crates/acp-agent-core/src/infrastructure`로 이동 (acp/* 전체, agent_session_registry, permission_broker, agent_catalog)
- [ ] T012 `json_acp_session_store`의 Tauri 경로 결합 제거: noop store 경로로 전환하고 `AcpSessionStore` 포트만 코어에 유지 (참조: research.md R7)
- [ ] T013 동시 실행 상한 env `ACP_WORKBENCH_MAX_RUNS` → `ACP_MAX_RUNS` 리네임(구 이름 하위호환) in `crates/acp-agent-core/src/infrastructure/agent_session_registry.rs`
- [ ] T014 `crates/acp-agent-core/src/lib.rs` 공개 API 정리 (RunEvent, AgentRunRequest, UseCase들, AcpAgentRunner, AppState, RunEventSink re-export)
- [ ] T015 [Required test] RunEvent camelCase 직렬화 라운드트립 픽스처 테스트 (계약 CT-1) in `crates/acp-agent-core/src/domain/events.rs`
- [ ] T016 [Required test] registry 동시 상한 및 소유자 기반 run 취소 테스트 (계약 CT-4·CT-5) in `crates/acp-agent-core/src/infrastructure/agent_session_registry.rs`
- [ ] T017 `packages/agent-client` 생성 및 workbench `entities/agent-run`의 types/repository 이동·re-export (`packages/agent-client/src/{types.ts,repository.ts,index.ts}`)
- [ ] T018 [Required test] TS↔Rust 계약 스냅샷 테스트 (RunEventEnvelope/AgentRunRequest camelCase) in `packages/agent-client/src`
- [ ] T019 `agentic-workbench`가 `acp-agent-core` + `@yoophi/agent-client`를 소비하도록 교체 (`apps/agentic-workbench/src-tauri`, `apps/agentic-workbench/src/entities/agent-run`)
- [ ] T020 [Atomic verify] `cargo test -p acp-agent-core`, `cargo test -p agentic-workbench`, `pnpm --filter @yoophi/agentic-workbench check-types` 그린 (원칙 V)
...
- [ ] T021 [US1] `pnpm build` + `pnpm check-types`가 hushline 포함해 통과함을 확인·수정
- [ ] T022 [US1] `cargo test -p hushline` 통과 확인·수정
- [ ] T023 [US1] 수동 검증: hushline 실행 후 YouTube URL 자막 생성, queue 추가/삭제, 진행 표시 회귀 없음 (quickstart V1)
