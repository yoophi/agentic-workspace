...
- [X] T017 `packages/agent-client` 생성(계약 타입 + invoke 래퍼 + Tauri listen 기반 listenRunEvents). workbench `entities/agent-run/model/types.ts`가 계약 타입을 패키지에서 re-export(소비처 28곳 무변경). workbench 동작 민감 repository는 로컬 유지
- [X] T018 [Required test] TS 계약 스냅샷 테스트 (`packages/agent-client/src/contract.test.ts`, 3 pass — AgentRunRequest/RunEventEnvelope/tool camelCase)
- [X] T019 `agentic-workbench`가 `acp-agent-core`(src-tauri, mod.rs re-export) + `@yoophi/agent-client`(프론트 타입 re-export) 소비하도록 교체
- [X] T020 [Atomic verify] Rust: `cargo test -p acp-agent-core`(84), `cargo test -p agentic-workbench`(113), `cargo check --workspace`(green). TS: `pnpm check-types`(11/11), `pnpm test`(agent-client 3 + workbench 267 등 전체 green)
...
- [X] T024 [US2] `Cargo.toml`에 `acp-agent-core` 의존 추가 및 `AppState` manage 등록 in `apps/hushline/src-tauri/src/adapters/tauri.rs`
- [X] T025 [US2] `HushlineAgentSink`(RunEventSink 구현) 작성 — `agent-run-event` 채널 emit in `apps/hushline/src-tauri/src/adapters/tauri.rs`
- [X] T026 [US2] `start_agent_run` command 등록 + 입력 검증(`goal` 비어있지 않음, `cwd` 관리 폴더 하위) + `generate_handler!` 반영 in `apps/hushline/src-tauri/src/adapters/tauri.rs`
- [X] T027 [P] [US2] [Required test] cwd 경로 검증 거부 테스트 (계약 CT-2) in `apps/hushline/src-tauri/src`
- [X] T028 [US2] `OrganizedDocument` 도메인 + 저장 포트/어댑터 (경로·크기·UTF-8 검증) in `apps/hushline/src-tauri/src/{domain,application,adapters}`
- [X] T029 [US2] 단일 기본 agent 실행 카탈로그/명령 최소 설정 제공 in `apps/hushline/src-tauri/src/adapters`

### 프론트 (apps/hushline/src)

- [X] T030 [P] [US2] `package.json`에 `@yoophi/agent-client` 의존(`workspace:*`) 추가 in `apps/hushline/package.json`
- [X] T031 [US2] `shared/api`에 agent-client 재노출 (startAgentRun, cancelAgentRun, listenRunEvents) in `apps/hushline/src/shared/api`
- [X] T032 [P] [US2] `entities/organized-document` 모델·API 어댑터 추가 in `apps/hushline/src/entities/organized-document`
- [X] T033 [US2] `features/organize-transcript`: `useAgentRun` 훅(startAgentRun + listenRunEvents로 AgentMessage 누적) in `apps/hushline/src/features/organize-transcript`
- [X] T034 [US2] result-grid 카드에 "정리하기" 액션 + 정리 방식 입력 UI in `apps/hushline/src/widgets/result-grid`, `apps/hushline/src/features/organize-transcript`
- [X] T035 [US2] AgentMessage 스트림 누적 → OrganizedDocument 저장 및 목록 재열람 UI in `apps/hushline/src/{features/organize-transcript,widgets/result-grid}`
- [X] T036 [US2] 취소/오류 처리 UI (cancelAgentRun 연결, `Error`·`Lifecycle(Cancelled)` 이벤트 표시) in `apps/hushline/src/features/organize-transcript`
- [~] T037 [US2] 검증: hushline check-types/build 그린, 전체 워크스페이스 11/11. 실제 스트리밍(V2)·저장(V3)·취소(V4) 런타임 확인은 agent CLI(claude 등) 설치 필요 — 배선/컴파일은 검증됨
...
- [X] T038 [US3] `send_prompt_to_run`, `respond_agent_permission`, `set_run_permission_mode` command 등록 + `generate_handler!` 반영 in `apps/hushline/src-tauri/src/adapters/tauri.rs`
- [X] T039 [P] [US3] [Required test] 미소유 `runId`에 대한 프롬프트/취소 거부 테스트 (계약 CT-3) in `apps/hushline/src-tauri/src`
- [X] T040 [US3] `ChatSession` 도메인 + 저장(메시지 로그) in `apps/hushline/src-tauri/src/{domain,application,adapters}`
- [X] T043 [US3] 창/앱 종료 시 소유 run 정리 연결 (계약 CT-4 hushline 측) in `apps/hushline/src-tauri/src/adapters/tauri.rs`

### 프론트 (apps/hushline/src)

- [X] T041 [US3] `features/chat-with-document`: 세션 시작 + `sendPromptToRun` 반복 + 스트림 렌더 in `apps/hushline/src/features/chat-with-document`
- [X] T042 [US3] 문서 뷰/result-grid에 "대화하기" 액션 + 대화 UI(메시지 목록·입력) in `apps/hushline/src/{widgets,features/chat-with-document}`
- [~] T044 [US3] 검증: check-types/build 그린, cargo test 6 pass(CT-3 소유 포함), 창종료 정리 배선(CT-4). 실제 다회 대화(V5) 런타임은 agent CLI 필요
...
- [X] T045 [P] `docs/20260721-acp-agent-core-reuse-strategy.md`에 실제 편입/추출 결과 반영 갱신
- [X] T046 [P] 신규 재사용 UI가 있으면 Storybook atomic 분류(atoms/molecules/organisms/pages) 등록 in `apps/hushline/src/stories`
- [X] T047 [P] 전체 계약/픽스처 테스트 재실행 및 atomic cross-app 검증 결과 문서화 (`cargo test -p acp-agent-core`, `-p agentic-workbench`, `-p hushline`, `pnpm check-types`)
- [X] T048 [P] 루트 `README.md` Apps 표에 hushline 추가 및 프로젝트 구조 반영
