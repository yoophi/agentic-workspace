...
있다. 이 경우 run 화면은 실패 메시지를 표시하고 사용자가 별도 Settings 창을
열어 프로필을 수정할 수 있는 경로를 제공한다. Settings 창은 `Cmd+,` 단축어로도
열 수 있으며, 이미 열린 상태에서는 기존 창을 앞으로 가져온다. 실행 실패는 기존
세션 목록, worktree 정보, 권한 상태, 다른 설정값을 변경하지 않아야 한다.
